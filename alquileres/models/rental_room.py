...
class RentalRoom(models.Model):
    ...
    # Información Básica
    name = fields.Char(string='Room', required=True, copy=False, readonly=True, default='New')
...
class RentalRoomOccupancy(models.Model):
    ...
    name = fields.Char(string='Room Occupancy History', required=True, copy=False, readonly=True, default='New')
    ...
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('rent.room.occupancy') or 'New'
...
class RentalRoomMaintenance(models.Model):
    ...
    name = fields.Char(string='Room Maintenance History', required=True, copy=False, readonly=True, default='New')
    room_id = fields.Many2one('rental.room', string='Room', required=True)
    ...
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('rent.room.maintenance') or 'New'
