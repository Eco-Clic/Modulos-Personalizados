...
class Repair(models.Model):
    ...
    adha_qr_code = fields.Binary("QR Code", compute='create_repair_in_adha_generate_qr_code', store=True)
    @api.depends('adha_qr_code')
    def create_repair_in_adha_generate_qr_code(self):
        print('\n\n\n')


        pattern = r"\[([^\]]+)\]"
        print("Create repairing in adha")
        data = {
            "nombre": "",
            "tipo_equipo": "",
            "otros_datos": "",
            "centro": "",
            "tipo_activo": "reparación", # Usado para crear el tipo activo en adha
            "cliente": {}
        }

        line_references = []
        for record in self:
            print("recdirdasdkjh -> ", record.name)
            print("recdirdasdkjh -> ", record.operations)
            # return
            for line in record.operations:
                print("line --> ", line.display_name)
            ...
            if self.env.context.get('from_button_click', False):
                ...
                self.save_qr_in_adha(id_equipo_adha)

        print('\n')
        return True

    def save_qr_in_adha(self, id_equipo_adha):
        print("Invocando la otra function")
        self.ensure_one()

        print("Save QR into adha")
        binary_pdf, type_file = self.env.ref('repair.action_report_repair_order')._render_qweb_pdf("repair.report_repairorder2", self.id)
        print("binary_pdf --> ", binary_pdf)
        
        pdf_buffer = BytesIO(binary_pdf)
        files = {'binary_pdf': ('document.pdf', pdf_buffer, 'application/pdf')}

        print("data_pdf --> ", files)
        response_data_pdf = requests.post(f"{ADHA_API_URL}/{id_equipo_adha}/equipos/guardar-documento/?is_binary=true&tipo_odoo=reparacion",
            files=files,
        )
    ...
    def _get_adha_qr_code(self):
        ...
        qr_to_return = f"data:image/png;base64,{self.adha_qr_code.decode('utf-8')}"
        return qr_to_return
    ...
    def envio_whatsapp(self):
        ...
        # Esta validación es si se da el caso de que el número movil esté en el campo phone y no mobile
        # Descomentarla para que funcione
        # parner_phone_number = self.partner_id.mobile if self.partner_id.mobile else self.partner_id.phone
        parner_phone_number = self.partner_id.mobile
        if not parner_phone_number:
            return False
